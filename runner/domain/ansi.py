...
un fond sombre choisi, c'est donc a la palette du theme de decider de la
teinte.
"""
...
def _couleur_256(index: int) -> str | None:
    """Couleur du mode 256 : 16 couleurs de base, un cube 6x6x6, 24 gris."""
    if index < 8:
    ...
    while i < len(parametres):
        ...
        if code == 0:
            ...
            if mode == 5 and i + 2 < len(parametres):
                style = replace(style, couleur=_couleur_256(parametres[i + 2]),
                                vive=False)
                i += 2
